...
/// `TypeEncoder` facilitates the encoding of `Encodable` values into property lists.
open class TypeEncoder {
    ...
    func encodeToTopLevelContainer<Value : Encodable>(_ value: Value) throws -> Any {
        let encoder = _TypeEncoder(options: self.options)
...
private class _TypeEncoder : Encoder {
    // MARK: Properties
    
    /// The encoder's storage.
    fileprivate var storage: _TypeEncodingStorage
    
    /// Options set on the top-level encoder.
    fileprivate let options: TypeEncoder._Options
    ...
    fileprivate init(options: TypeEncoder._Options, codingPath: [CodingKey] = []) {
        self.options = options
        self.storage = _TypeEncodingStorage()
    ...
    public func container<Key>(keyedBy: Key.Type) -> KeyedEncodingContainer<Key> {
        ...
        let container = _TypeKeyedEncodingContainer<Key>(referencing: self, codingPath: self.codingPath, wrapping: topContainer)
    ...
    public func unkeyedContainer() -> UnkeyedEncodingContainer {
        ...
        return _TypeUnkeyedEncodingContainer(referencing: self, codingPath: self.codingPath, wrapping: topContainer)
    }
...
fileprivate struct _TypeEncodingStorage {
    // MARK: Properties
    
    /// The container stack.
    /// Elements may be any one of the plist types (NSNumber, NSString, NSDate, NSArray, NSType).
...
fileprivate struct _TypeKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    typealias Key = K
    
    // MARK: Properties
    
    /// A reference to the encoder we're writing to.
    private let encoder: _TypeEncoder
    ...
    fileprivate init(referencing encoder: _TypeEncoder, codingPath: [CodingKey], wrapping container: NSMutableDictionary) {
    ...
    public mutating func encodeNil(forKey key: Key)               throws { self.container[key.stringValue] = _dictNullNSString }
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type, forKey key: Key) -> KeyedEncodingContainer<NestedKey> {
        ...
        let container = _TypeKeyedEncodingContainer<NestedKey>(referencing: self.encoder, codingPath: self.codingPath, wrapping: dictionary)
        return KeyedEncodingContainer(container)
    }
    ...
    public mutating func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
        ...
        self.codingPath.append(key)
        defer { self.codingPath.removeLast() }
        return _TypeUnkeyedEncodingContainer(referencing: self.encoder, codingPath: self.codingPath, wrapping: array)
    }
    
    public mutating func superEncoder() -> Encoder {
        return _TypeReferencingEncoder(referencing: self.encoder, at: _TypeKey.super, wrapping: self.container)
    }
    
    public mutating func superEncoder(forKey key: Key) -> Encoder {
        return _TypeReferencingEncoder(referencing: self.encoder, at: key, wrapping: self.container)
    }
}

fileprivate struct _TypeUnkeyedEncodingContainer : UnkeyedEncodingContainer {
    // MARK: Properties
    ...
    /// Initializes `self` with the given references.
    fileprivate init(referencing encoder: _TypeEncoder, codingPath: [CodingKey], wrapping container: NSMutableArray) {
        self.encoder = encoder
        self.codingPath = codingPath
        self.container = container
    }
    ...
    public mutating func encodeNil()             throws { self.container.add(_dictNullNSString) }
    ...
    public mutating func encode<T : Encodable>(_ value: T) throws {
        self.encoder.codingPath.append(_TypeKey(index: self.count))
        defer { self.encoder.codingPath.removeLast() }
        self.container.add(try self.encoder.box(value))
    }
    
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type) -> KeyedEncodingContainer<NestedKey> {
        self.codingPath.append(_TypeKey(index: self.count))
        defer { self.codingPath.removeLast() }
    ...
    public mutating func superEncoder() -> Encoder {
        return _TypeReferencingEncoder(referencing: self.encoder, at: self.container.count, wrapping: self.container)
    }
}

extension _TypeEncoder : SingleValueEncodingContainer {
    ...
    public func encodeNil() throws {
        ...
        self.storage.push(container: _dictNullNSString)
    }
...
extension _TypeEncoder {
    ...
    fileprivate func box_<T : Encodable>(_ value: T) throws -> NSObject? {
        ...
        // The value should request a container from the _TypeEncoder.
...
/// _PlistReferencingEncoder is a special subclass of _TypeEncoder which has its own storage, but references the contents of a different encoder.
/// It's used in superEncoder(), which returns a new encoder for encoding a superclass -- the lifetime of the encoder should not escape the scope it's created in, but it doesn't necessarily know when it's done being used (to write to the original container).
fileprivate class _TypeReferencingEncoder : _TypeEncoder {
    ...
    fileprivate init(referencing encoder: _TypeEncoder, at index: Int, wrapping array: NSMutableArray) {
        self.encoder = encoder
        self.reference = .array(array, index)
        super.init(options: encoder.options, codingPath: encoder.codingPath)
        
        self.codingPath.append(_TypeKey(index: index))
    }
    
    /// Initializes `self` by referencing the given dictionary container in the given encoder.
    fileprivate init(referencing encoder: _TypeEncoder, at key: CodingKey, wrapping dictionary: NSMutableDictionary) {
        self.encoder = encoder
...
fileprivate let _dictNull = "$null"
fileprivate let _dictNullNSString = NSString(string: _dictNull)
